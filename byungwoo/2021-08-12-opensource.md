...
[물론 사람마다 오픈소스에 기여하려는 이유는 다양할 것이다.](https://haksung.gitbook.io/oss/research/contribution/why-devoloper-contribute)
...
## 오픈소스 기여의 장벽
오픈소스 개발을 시작하고자 '오픈소스 기여'라고 검색하면 다음과 같은 결과가 나옵니다.
...
물론 앞서 언급한 사전지식과 기여방법은 오픈소스 기여활동을 하는 데에 중요한 지침이 되지만 원론적인 이야기를 하는 경우가 많다고 생각합니다.
...
- Pull Request: 내 저장소의 변경 내용을 다른 사람의 저장소에 반영하도록 요청하는 것. ('목적'이 아니라 '행위'로 표현한 용어여서 헷갈리기 쉽다.)
...
## 오픈소스 기여 방법
GitHub를 활용한 오픈소스 기여는 다음의 3단계로 이루어집니다.

## 1. Fork하기
![img_5.png](img_5.png)
관심이 있거나 기여하고 싶은 프로젝트(upstream)를 찾아서 우측의 포크(🍴)로 내 계정의 원격 저장소(origin)로 복제합니다.

![img_6.png](img_6.png)
<center>저장소를 fork하면 이렇게 표시됩니다.</center>
...
(개인적으로는 규모가 클수록 contributor와 issue가 너무 많고 프로젝트 구조를 파악하는 데에 오래 걸리기 때문에 처음에는 작은 프로젝트가 좋은 것 같습니다.)

언제든지 바꿀 수 있는 것이기 때문에 관심이 생기는 프로젝트면 일단 fork해서 들여다 보는 것을 추천합니다. 

나의 원격 저장소(origin)로 fork한 후에는 내가 원하는 대로 수정을 해도 본래의 저장소(upstream)에는 직접적으로 영향을 주지 않으니 마음껏 건드려보고 아니다 싶으면 저장소를 삭제하면 됩니다.


## 2. Issue 들여다보기/작성하기
프로젝트는 fork했으면 이제 들여다볼 차례입니다.
...
Issue를 들여다보는 것도 영어라서 어렵고 지루하다 싶으면 가장 쉽게 접근할 수 있는 오탈자 찾기를 진행합니다.

앞서 로컬에 clone한 프로젝트에서 오탈자가 있는지 찾아봅니다. 오탈자뿐만 아니라 들여쓰기가 이상한 곳을 수정하거나 어색한 표현을 바꾸는 것으로도 기여를 할 수 있습니다.

수정하고 싶은 파일을 GitHub에서 클릭하면 우측에 있는 🖊️버튼을 수정이 가능하며 git에 익숙할 경우 직접 수정하여 git commit & push로 내 원격 저장소(origin)에 반영합니다.
...
PR 과정에서 여러가지 코멘트와 요구사항이 추가될 수 있습니다.

수정사항 반영이 필요할 경우에는 PR을 진행하고 있는 나의 저장소(origin)의 브랜치(저의 경우는 `bart2001/exporterhub.io` 저장소의 `issue/72` 브랜치)에서 파일을 GitHub에서 수정하거나 수정사항을 commit & push를 반영해주면 됩니다.

요구사항을 모두 충족했다면 오픈소스 메인테이너가 `Confirm Merge` 버튼을 눌러서 메인 저장소에 드디어 반영이 됩니다! 
...
## (추가) Fetch
하나의 오픈소스 프로젝트에는 여러 사람들이 기여합니다. 다시 말해서 내가 PR로 본 저장소(upstream)에 변경사항을 반영하는 것뿐만 아니라 다른 사람들의 PR에 의해서 나의 저장소(origin)와 본 저장소(upstream)의 형상은 계속 달라질 수 있습니다. 따라서 fetch 기능을 통해서 지속적으로 본 저장소(upstream)와 나의 저장소(origin)의 상태를 맞추어 주어야 나의 변경사항을 충돌없이 PR을 통해서 보낼 수 있습니다. 

나의 저장소(origin)의 우측에 있는 `🔄Fetch upstream`을 눌러서 본 저장소(upstream)와 나의 저장소(origin)의 상태를 맞추어 줍니다. 
![img_14.png](img_14.png)

물론 git 명령어가 익숙하다면 git fetch와 merge 명령어로 동일하게 수행할 수 있습니다.

(여기서 충돌이 생기는 경우는 git에 대한 어느 정도의 이해가 필요하고 이 글의 범위를 벗어나므로 다루지는 않겠습니다.)

## 마무리
조금은 장난스럽게 표현한 감이 있지만 제가 오픈소스 프로젝트에 생애최초(?)로 기여한 경험을 짧게 공유하였습니다. 오해가 소지가 있을 것 같아서 다시 한 번 말씀드리면 오픈소스 기여가 단순히 오탈자를 찾아 고치는 행위는 아닙니다. 오픈소스 프로젝트 기여가 어렵고 막연하다고 느끼지 않도록 작은 개선사항부터 반영하는 것에서 흥미를 느끼고 시작할 수 있었으면 좋겠습니다. (갑자기 마무리가 진지해져서 어색합니다.)  
