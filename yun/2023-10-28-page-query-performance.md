...
Slice, Page 의존성은 Spring Data Commons에 있기 떄문에 Spring Data MongoDB에서도 동일하게 적용할수 있습니다.
...
```
`applyPagination`, `applySlicePagination` 모두 동일한 메커니즘으로 구현되어 있습니다. 구현 쿼리는 람다로 받고 `applyPagination`는 content 쿼리와, count 쿼리를 코루틴을 통해 비동기로 가져오고 `applySlicePagination`에서는 hasNext를 구현하고 있습니다.
...
`MongoCustomRepositorySupport`를 상속 받으며 필요한 의존성인 `documentClass`, `mongoTemplate`을 넘겨줍니다. 페이징 구현이 필요한 곳에서 `MongoCustomRepositorySupport`에서 구현한 `applyPagination`, `applySlicePagination`을 사용합니다. 