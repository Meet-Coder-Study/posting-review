...
매뉴얼 테스팅에서의 포함된 일들을 보면, 각 트랜잭션을 불러오고 각 시나리오테스트의 성공 여부를 리포트 하며, 폼이 있다면 폼의 유효성 검사를 스크린샷으로 찍습니다. 이런 브라우저 테스팅을 실제 사용자환경과 동일한 조건에 맞추어 실제 브라우저에서 자동화하여 여러 브라우저를 한꺼번에 테스팅을 실현하게 도와주는 것이 headles browser를 이용하는 것입니다.  
