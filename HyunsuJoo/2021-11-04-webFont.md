## Gatsby 구글 웹 폰트에서 셀프 호스트 최적화 적용기
...
<img src="./images/webfont_problem.png" alt="web_font_issue" width="500px">
<img src="./images/googlefont_lighthouse.png" alt="issue" width="500px">
위의 리소스들은 waterfall 이다. font.googleapis.com 작업에, blocking rendering 이 일어난다. 그 다음 리소스인 로컬 폰트를 다운로드 하는 작업이 끝날 지점에 일어난다.
문제점을 정리하자면,

1. 렌더링 차단
2. “Avoid chaining critical request” error - Lighthouse
3. 페이지 전환시 font가 깜빡 거리는 현상

## 렌더링 차단과 Google web fonts 동작원리의 관계

`link` 태그를 구분 분석하고 stylesheet를 불러온다. href 속성을 로드하면 google font는 우리가 요청한 글꼴을 파악해 가져올 수 있는 폰트를 가져오는데 stylesheet 내에 @font-face로 폰트들을 정의하여 stylesheet를 브라우저로 보낸다. (`rel=stylesheet`)
그런 다음, 다시 @font-face에 정의된 local version 글꼴을 확인한다. 만약 없다면 fonts.gstatic.com에 연결하여 실제 글꼴을 다운로드한다.
...
“Avoid chaining critical request” 에러는 웹폰트 로드 시 렌더링 차단을 짚어준다. 브라우저는 페이지 로드 프로세스가 시작될 때 HTML을 파싱할 때 할당된 우선순위에 따라 중요한 요청을 한다. 지정하지 않는 한 HTML은 일반적으로 가장 높은 우선순위를 가지며 CSS, 이미지 및 JavaScript의 순서인데 이런 요청들이 긴 체인을 형성하게 되면 지연시간이 늘어나고 이는 페이지 로드 시간을 늘어나게 된다.
...
- preconnect
  ...
  preconnect는 외부 도메인과의 연결을 미리 연결을 설정합니다. 그래서 연결된 설정으로 리소스를 가져올 수 있으므로 DNS, TCP, TLS 왕복에 필요한 시간을 절약할 수 있다.
...
렌더링 차단이 없어진 덕에 초기 렌더링이 이전 7sec -> 2.5s~3s 로 단축되었다.
...
<img src="./images/woff2.png" alt="woff2" width="300px"/>

#### woff

<img src="./images/woff.png" alt="woff" width="300px"/>

#### EOT

<img src="./images/eot.png" alt="eot" width="300px"/>

#### TTF

<img src="./images/eot.png" alt="ttf" width="300px"/>
...
<img src="./images/preload.png" alt="preload" width="300px"/>

#### preconnect

<img src="./images/preload.png" alt="preconnect" width="300px"/>
...
같은 유저가 https://b.example 에 방문하고 같은 이미지를 요청합니다. 기존 캐시 시스템과는 달리 the top-level site 캐시 키가 다르므로, 새로 캐시로 저장한다.

- Cache Key: { https://b.example, https://b.example, https://x.example/doge.png }

같은 유저가 이번엔 https://a.example를 방문하여 (https://x.example/doge.png)를 요청합니다. 같은 top-level site 키와 the iframe 키가 있을 시 같은 리소스로 간주하고 캐시 된 리소스를 반환한다.
...
> 서브셋 폰트는 일부 글리프나 영약만을 추출하여 새로운 폰트를 추출하는 것을 이야기합니다.
...
preload로 인해서 사용하지 않는 자원까지 미리 로드하는 것은 낭비 일 수 있으며, 렌더링이 된 후에도 폰트를 다운로드되는 과정이 이어지기 때문에 그때까지는 아무것도 보이지 않는 FIOT 현상이 일어날 수 있다.
처음 google Light에서 받은 에러로 `“ensure text remains visible during web font load”`가 FIOT 현상을 이야기한다.

브라우저마다 웹페이지에 큰 폰트 파일을 로드하는 과정에서 생기는 지연을 처리하는 방식이 조금 다른데, FOIT, FOUT가 있다.
FOIT는 글꼴을 로드할 준비가 되지 않은 경우 일부 브라우저는 텍스트를 완전히 숨긴다. Chrome이나 Firefox는
3초 동안 텍스트를 숨기고 그때까지 폰트가 로드되지 않으면 시스템 폰트가 로드된다. Safari의 경우는
모든 글꼴을 로드할 때까지 텍스트를 완전히 숨긴다.

FIOT(Flash of Invisible Text)는 `@font-face` 에서 `font-display: swap` 속성을 이용하여 FOUT로 전환하는데, 이는 사용자 입장에서는 아무것도 보지 않는 것보다 빠르게 시스템 폰트를 먼저 보고 폰트 다운로드가 완료된 후에 다운로드된 폰트를 보게 되는 것이다.

[font-dsplay의 속성 값과 그에 따른 차이에 대한 자세한 설명 - naverD2-웹 폰트 사용과 최적화의 최근 동향](https://d2.naver.com/helloworld/4969726)
...
<img src= "./images/display_swap_before.png" width= "400px" height="300px" alt="display_swap_before">
...
셀프 호스팅 폰트에서 한글 폰트가 라틴폰트에 비해 10배 정도 차이지만, 첫 렌더링에서는 셀프 호스팅이 300ms정도 빠른 걸로 확인할 수 있다. 그렇다고 셀프 호스팅이 무조건 빠르다고 결론을 내릴 순 없는 것 같다. 현재 사용 중인 폰트 수가 많지 않으며, 테스트마다 매번 같은 결과를 낼 수도 없고 preloading이 되는 리소드들이 많아지거나 하면 결과는 충분히 달라지기 때문이다. 하지만 알 수 있는 것은 현재 블로그 내에서 다양한 폰트를 사용하지 않을 경우, 많은 리소스들이 있지 않은 경우에는 셀프 호스팅과 구글 웹폰트를 최적화할 경우 비슷한 퍼포먼스를 도출한다고 볼 수 있다.

## 마무리

웹폰트 주제가 알면 알 수록 재미있고 생각보다 깊은 주제였다. 모르는 더 많은 것들이 있지만 나의 gatsby 블로그에서의 웹폰트 최적화는 전반적인 성능 향상에 크게 기여가 되었다.
만약 Google 웹폰트를 사용한다면 폰트의 경량화 작업을 생각할 필요도 없고 어떤 버전을 결정하는 데 도움이 되도록 브라우저 지원을 확인하는 번거로움도 없을 것이다. 하지만, 캐시 성능닫기캐시성능 면에서 보았을 때도 그랬지만 더 이상닫기더이상 google 웹폰트가 캐시로 더 빠르게 제공한다는 이점도 없는 것이다. 또한 더 이상닫기더이상 구글로부터 내 클라이언트들의 정보를 자유롭게 하고 싶다면 셀프 호스팅이 낫지 않을까 싶다.
