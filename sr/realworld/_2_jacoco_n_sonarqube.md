...
생각하지 못했던 일들로 원하는 내용을 전부 담지는 못했다.

일단 정리해보면 RealWorld를 시작할 때 기존에 아무생각 없이 구현 코드를 작성하는 식의 개발하던 방식에서 

최대한 `TDD 방식`으로, `코드 커버리지 도구`를 도입해서 하면 처음 하는 설정에 신경 많이 쓰게 될 것 같아 `TDD 방식`으로 개발하는 것에만 신경 썼었다.

하지만 도입해보니 `Jacoco`나 `SonarQube`는 테스트를 작성하는 데 더 도움을 주는 도구였었다.

특히나 Jacoco는 테스트를 실행하면 HTML로 바로 확인하여 로직에 대해서 어떠한 부분에서 테스트가 미흡했는지 확인할 수 있었다.

이러한 도구를 바로 적용하고 했다면 조금 더 빨리 개발하고 문제를 개선하면서 진행했을 것 같다.

앞으로도 레거시 코드를 작성하게 되는 이런 습관을 좀 더 객체지향적인 설계를 할 수 있도록 개념과 그를 돕는 도구를 통해 이야기 할 수 있을 것 같다.