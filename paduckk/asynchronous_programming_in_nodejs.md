...
1번 예시는 완전하지 않지만 함수를 setTimeOut 함수의 첫번째 인자에 1초뒤에 실행 될 함수를 올바르게 넘겨 주었습니다 하지만 (10, 30) 인자의 값은 넘겨주지 못해 NaN이 나올 것 입니다.
...
이러한 패턴은 콜백 패턴에서 많이 쓰이는 방법으로 DOM 오브젝트에 이벤트 핸들러를 붙일 때도 자주 사용하는 패턴입니다.

코드로 직접 처보시며 이해 해 보시는 것이 중요합니다 👍
