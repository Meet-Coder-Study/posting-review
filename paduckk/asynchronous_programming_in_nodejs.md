...
위에는 파일을 읽어와 콘솔에 출력하는 의사코드 입니다. 파일을 읽은 후 콘솔에 파일 변수를 출력하기 위해선 파일 읽기 작업이 모두 끝난 후에 `file` 변수에 담아 콘솔로 출력을 하여야 합니다.

이 의사 코드를 Blocking IO로 제어 할 때와 NonBlocking IO로 제어 할 때의 예시를 살펴 보겠습니다.
...
java sdk의 File Io 라이브러리를 사용해서 파일을 읽은 후 콘솔에 출력 하였습니다. 갑자기 자바코드가 나와 당황 하신분들은 자바 내장 파일 입출력을 기능을 사용시 코드의 진행이 막혔다가 파일을 다 읽은 후에 다음 코드가 실행 된다고 이해 하시면 됩니다.
...
여기서 `loop`은 동일한 행위를 계속해서 반복하는 것을 말합니다. 단일 스레드 이벤트 루프는 무엇을 계속해서 반복한다는 것인지 비유해서 설명해보록 하겠습니다.

- 단일 스레드 이벤트 루프 => `상자 뒤지는 일꾼`
- 이벤트 => 사건 (ex: 파일 읽기 완료!, enter 키 입력, 네트워크 요청)
- 큐 => 상자
- 함수 => 일

1. `상자 뒤지는 일꾼`은 쉬지 않고 상자에 할 일이 담겨있나 확인하고 담겨있으면 얼른 처리 합니다.
2. `상자 뒤지는 일꾼`에게 './test.txt' 파일을 읽고 읽은 데이터를 `콘솔에 출력하라는 일`을 주었습니다.
3.  `상자 뒤지는 일꾼`은 파일을 읽는 일은 자기가 할 수 있는 일이 아니므로 파일 읽기 담당 일꾼에게 떠넘깁니다 이 때 `파일 읽은 후 할 일`도 같이 넘깁니다.
4. 파일 읽기 담당 일꾼은 열심히 파일을 읽은 뒤! 파일에서 얻은 데이터를 `콘솔에 출력하는 일`을 실행 하지 않고! 상자에 담아 둡니다.
5. 상자에 담긴 일은 `상자 뒤지는 일꾼`이 담당입니다. 상자에 담긴 `콘솔에 출력하는 일`을 처리하고 다음 상자를 반복해서 뒤집니다.


Nodejs는 스레드를 블락킹하고 지연이 예측 되는일들은 단일 스레드에서 처리하지 않고 미리 만들어둔 스레드나 c/c/c++로 작성된 성능이 더 좋은 모듈을 통해서 처리하도록 강제합니다.

자바스크립트 코드를 처리하는 메인 스레드는 단순 반복문, 함수 호출, 자바스크립트 연산 작업들을 처리 합니다.

Nodejs가 연산과 계산이 많이 들어가는 경우 좋지 않다고 하는 경우가 말하는 이유가 여기에 있습니다. 

하나의 콜백 함수가 많은 연산 작업을 포함하고 있다면 단일 스레드가 이 함수를 처리하기 전까지 다른 이벤트 큐에 쌓여 있는 일들을 처리하지 못하기 때문입니다.
