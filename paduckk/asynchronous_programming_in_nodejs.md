...
loop은 동일한 행위를 계속해서 반복하는 것을 말합니다. 

단일 스레드 이벤트 루프는 무엇을 계속해서 반복한다는 것인지 비유해서 설명해보록 하겠습니다.
...
- `상자 뒤지는 일꾼`은 쉬지 않고 상자에 할 일이 담겨있나 확인하고 담겨있으면 얼른 처리 합니다.
- `상자 뒤지는 일꾼`에게 './test.txt' 파일을 읽고 읽은 데이터를 `콘솔에 출력하라는 일`을 주었습니다.
- `상자 뒤지는 일꾼`은 파일을 읽는 일은 자기가 할 수 있는 일이 아니므로 파일 읽기 담당 일꾼에게 떠넘깁니다 이 때 `파일 읽은 후 할 일`도 같이 넘깁니다.
- 파일 읽기 담당 일꾼은 열심히 파일을 읽은 뒤! 파일에서 얻은 데이터를 `콘솔에 출력하는 일`을 실행 하지 않고! 상자에 담아 둡니다.
- 상자에 담긴 일은 `상자 뒤지는 일꾼`이 담당입니다. 상자에 담긴 `콘솔에 출력하는 일`을 처리하고 다음 상자를 반복해서 뒤집니다.


Nodejs는 스레드를 블락킹하고 지연이 예측 되는일들은 단일 스레드에서 처리하지 않고 미리 만들어둔 스레드나 OS의 api를 통해서 처리하도록 강제합니다.

단일 스레드 자바스크립트 코드를 처리하는 메인 스레드는 단순 반복문, 함수 호출, 자바스크립트 연산 작업들을 처리 합니다.
