...
var foo = 42 // 한 번에 여러 var 선언 및 초기화
foo := 42 // 축약형, func 본문에서만 var 키워드 생략, 유형은 항상 암시적
...
// 메서드로 선언하기를 원하는 struct(수신 유형)는 func 키워드와 메소드 이름 사이에 있다.
...
```go
// 오류 내장 인터페이스 유형은 오류 조건을 나타내는 일반적인 인터페이스이며, 
...
```go
ch := make(chan int) // int 유형의 채널 생성
