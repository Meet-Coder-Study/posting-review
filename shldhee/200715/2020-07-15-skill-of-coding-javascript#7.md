...
- 위와 같은 장점들이 있지만 현실을 테스트를 작성하는것 쉽지 않다.(테스트 작성의 어려움, 코드가 외부 의존성이 강하게 결합 등..)
...
- 테스트할떄 어려운 부분은 바로 `getTaxInformation()` 외부 함수를 호출 할때 시작된다.
...
- 테스트 작성이 어렵다는데 그건 테스트를 작성하기 쉽게 코드를 변경해야 한다.
...
- 웹사트에 행사 안내 페이지가 있다고 가정하자. 아래 코드를 통해 살펴보자.
  - 장소, 건물, 관리자 등은 크게 달라지지 않지만,
  - 행사 내용이 달라질 수 있다.
...
- 첫번쨰, 두번째 매개변수는 (`building, manager`)로 항상 동일하다. 반복 호출 중이다.
